...
#define MAX7219_CS  15                            // Pin used for CS. 7=UNO 15=ESP8266
...
void MAX7219_Init() {
  ...
  MAX7219_All_Chips(MAX7219_Intensity, Intensity);
...
void Write_Row(uint8_t Register_Address, uint8_t Register_Data) {
  ...
  if (Register_Address >= 0 && Register_Address <= ((Number_MAX7219 * 8) - 1)) {
    ...
    // Find the chips data register to write data too, result is Row_Address. Range = 1 to 8 (data registers of the MAX7219
...
void Scroll_Update() {
  ...
  if (Scroll_Position < zz) {
    ...
    Scroll_Loops--;
    //    Serial.println(Scroll_Loops);

    // if finished scroll loops
    if (Scroll_Loops == 0) {
      Scrolling = false;
    }
...
// -------------------------
void Scroll_Text_Start(String Text, int a, int zz) {
...
// ---------------------------------------
// display a clock with or without a colon
// ---------------------------------------
void Display_Clock(String Text_Value, int Colon_Visable) {

  // Add a range check and display ???? if invaled
  if (Text_Value.length() < 5 && Text_Value.length() > 2) {

    int Column_Clock = 1;

    // test for 3 digit time
    ...
    for (int ll = 0; ll < Text_Value.length(); ll++) {
      ...
      // add space for the colon
      if (ll == 2 && Text_Value.length() == 4) {
        Column_Clock = Column_Clock + 3;
      }
      if (ll == 1 && Text_Value.length() == 3) {
        Column_Clock = Column_Clock + 3;
      }

      // the normal "Wirite_Character" uses 8 pixels wide, too wide for a clock. Hence the use of Column_Clock
      Column = Column_Clock;
      Column_Clock = Column_Clock + 7;

      Write_Character(Text_Value.charAt(ll));
    } // end of loop

    // add colon if enabled
    // for 4 digit clock display
    if (Colon_Visable == 1 && Text_Value.length() == 4) {
      // add a colon
      Frame_Buffer[9] = Frame_Buffer[9] | 0x80;
      Frame_Buffer[10] = Frame_Buffer[10] | 0x80;
      Frame_Buffer[12] = Frame_Buffer[12] | 0x80;
      Frame_Buffer[13] = Frame_Buffer[13] | 0x80;

      Frame_Buffer[17] = Frame_Buffer[17] | 0x01;
      Frame_Buffer[18] = Frame_Buffer[18] | 0x01;
      Frame_Buffer[20] = Frame_Buffer[20] | 0x01;
      Frame_Buffer[21] = Frame_Buffer[21] | 0x01;
    } // end of colon

    // for 3 digit clock display
    if (Colon_Visable == 1 && Text_Value.length() == 3) {
      // add a colon
      Frame_Buffer[9] = Frame_Buffer[9] | 0x30;
      Frame_Buffer[10] = Frame_Buffer[10] | 0x30;
      Frame_Buffer[12] = Frame_Buffer[12] | 0x30;
      Frame_Buffer[13] = Frame_Buffer[13] | 0x30;
  ...
  // display out of range
    ...
    Display_Text("????", 1);            // (text to display, location(pixels))
  } // end of else
...
// display temperature
// ---------------------------------------
void Display_Temperature(float Value, String C_F) {


  // add range check for temperature here

  Value = Value * 10;
  String Text_Value(Value, 0);
  //Serial.println(Text_Value);
  ...
  // d.p.
  Frame_Buffer[13] = Frame_Buffer[13] | 0b11000000;
  Frame_Buffer[14] = Frame_Buffer[14] | 0b11000000;

  if (C_F == "C") {
    int Cent = 24;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b00000011; Cent++;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b00000011; Cent++;
    
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b01111000; Cent++;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b11001100; Cent++;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b00001100; Cent++;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b11001100; Cent++;
    Frame_Buffer[Cent] = Frame_Buffer[Cent] | 0b01111000; Cent++;
  } // end if
